    ...
    class Wp_rem_received_enquiry_email_template {
        ...
        public function wp_rem_received_enquiry_email_callback( $form_fields = array() ) {
            ...
            if ( isset( $template['email_notification'] ) && $template['email_notification'] == 1 ) {
                ...
                $send_member_email = $this->get_member_email();
                if($send_member_email != ''){
                    $recipients = $recipients.','.$send_member_email;
                }
                $args = array(
                    'to' => $recipients,
                    'subject' => $subject,
        ...
        function get_member_email() {
            $property_member_id = isset( $this->form_fields['wp_rem_enquiry_member'] ) ? $this->form_fields['wp_rem_enquiry_member'] : '';
            $property_member_email = get_post_meta( $property_member_id, 'wp_rem_email_address', true );
